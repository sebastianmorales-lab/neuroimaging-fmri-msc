% Instrucciones para correr el fist level parametrico (modelando la pregunta) on pugliese

%addpath('/home/sebastian/maestria_task/MATLAB/spm12') %agraga la carpeta de spm a Matlab

function [matlabbatch] = first_level_parametric_question_job_fco(participant_folder, trigger_id)
...
disp(trigger_id);
%logfiles = 'D:\Expansion\maestria_task\rawdata' %en windows
logfiles = "/media/sebastian/Seagate Portable Drive/Expansion/maestria_task/rawdata";
%triggers_dir ='D:\Expansion\maestria_task\triggers' %en windows
triggers_dir = "/media/sebastian/Seagate Portable Drive/Expansion/maestria_task/triggers";
...
%base_dir = 'D:\Expansion\maestria_task\FMRI\Second_level_parametric_question_times\participantes';
%prepro_dir = 'D:\Expansion\maestria_task\FMRI\Prepro\';

base_dir = '/media/sebastian/Seagate Portable Drive/Expansion/maestria_task/FMRI/news/Second_level_parametric_question_timesByDecision/participantes';
prepro_dir = '/media/sebastian/Seagate Portable Drive/Expansion/maestria_task/FMRI/Prepro/';

disp(char(participant_folder));
output      = fullfile(base_dir, char(participant_folder), 'niftii');
mkdir(output);
inputs_base = fullfile(prepro_dir, char(participant_folder), 'niftii'); %Cambio la carpeta del input por la del prepro
...
for i_block=1:n_blocks
    ...
    % CORRECCIÓN: Calcular time_question DESPUÉS de definir bloques{i_block}
    % y guardarlo en una cell array para usarlo después
    time_question{i_block} = rmmissing(data_block.show_question);
...
[contrastes, all_contrastes] = deal(compute_contrastes_parametric_maestria_question_fco(choice_individual_blocks,outcomes_individual_b, outcomes_individual_m, repMB, repIndB, repIndM,posicionMB, posicionIndB, posicionIndM));
...
for i_block=1:n_blocks
    ...
    numero_condicion = 0; % Contador de condiciones en la matriz de diseño
    current_actual_predictor_count = 0; % Contador de predictores reales en la matriz de diseño
    
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).scans = scans{i_block};
    numero_condicion = numero_condicion +1;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).name = 'tiempos_decision_compet';
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).onset = choice_competitive_blocks{i_block}.show_game_time;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).duration = choice_competitive_blocks{i_block}.social_time;
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(1).param = choice_competitive_blocks{i_block}.rival;
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(2).param = choice_competitive_blocks{i_block}.pay_competitive;
    ...
    current_actual_predictor_count = current_actual_predictor_count + 3; % 1 onset + 2 pmods
    
    
    numero_condicion = numero_condicion +1;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).name = 'tiempos_decision_indiv';
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).onset = choice_individual_blocks{i_block}.show_game_time;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).duration = choice_individual_blocks{i_block}.social_time ;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).tmod = 0;
    
    
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(1).name = 'rival';
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(1).param = choice_individual_blocks{i_block}.rival;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(1).poly = 1;
    
    
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(2).name = 'pay_competitive';
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(2).param = choice_individual_blocks{i_block}.pay_competitive;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod(2).poly = 1;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).orth = 1;
    current_actual_predictor_count = current_actual_predictor_count + 3; % 1 onset + 2 pmods
    ...
    %Add questions
    numero_condicion = numero_condicion +1;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).name = 'show_question';
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).onset = time_question{i_block};  % CORRECCIÓN: usar el bloque específico
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).duration = 6;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).tmod = 0;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod = struct('name', {}, 'param', {}, 'poly', {});
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).orth = 1;
    current_actual_predictor_count = current_actual_predictor_count + 1; % 1 onset
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).duration = 3;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).tmod = 0;
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).pmod = struct('name', {}, 'param', {}, 'poly', {});
    matlabbatch{1}.spm.stats.fmri_spec.sess(i_block).cond(numero_condicion).orth = 1;
    current_actual_predictor_count = current_actual_predictor_count + 1; % 1 onset
   ...
   if length(outcomes_competitive_mb{i_block}.outcome_time)~= 0
        ...
        posicionMB(i_block) = current_actual_predictor_count + 1;
            repMB = repMB -1;
    end
    ...
    current_actual_predictor_count = current_actual_predictor_count + 1; % 1 onset
        ...
        if length(outcomes_individual_b{i_block}.outcome_time) ~= 0
            ...
            posicionIndB(i_block) = current_actual_predictor_count + 1;
            repIndB = repIndB -1;
        ...
        if length(outcomes_individual_m{i_block}.outcome_time) ~= 0
            ...
            posicionIndM(i_block) = current_actual_predictor_count + 1;
            repIndM = repIndM -1;
        end
...
contrastes_computed = compute_contrastes_parametric_maestria_question_fco(choice_individual_blocks,outcomes_individual_b, outcomes_individual_m,repMB, repIndB,repIndM,posicionMB, posicionIndB,posicionIndM);
