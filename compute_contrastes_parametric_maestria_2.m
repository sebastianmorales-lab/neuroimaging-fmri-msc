function [output] = compute_contrastes_parametric_maestria_2(choice_individual_blocks, outcomes_individual_b, outcomes_individual_m, repMB, repIndB, repIndM, posicionMB, posicionIndB, posicionIndM)
...
contrastes = {
   ...
   {'cJB>cJM'        [0  0  0  1  1 -1 -1  0  0]}                            %  4
   {'cojB>cojM'      [0  0  0  1 -1  1 -1  0  0]}                            %  5
   {'BB>BM'          [0  0  0  1 -1  0  0  0  0]}                            %  6
   {'BB>MB'          [0  0  0  1  0 -1  0  0  0]}                            %  7
   {'BB>MM'          [0  0  0  1  0  0 -1  0  0]}                            %  8
   {'BM>MB'          [0  0  0  0  1 -1  0  0  0]}                            %  9
   {'BM>MM'          [0  0  0  0  1  0 -1  0  0]}                            %  10
   {'MB>MM'          [0  0  0  0  0  1 -1  0  0]}                            %  11
   {'interaccion'    [0  0  0  1 -1 -1  1  0  0]}                            %  12
...
%       contrast_block(end+1) = 0;
%     end
  if i_contraste >= 1 && i_contraste <= 3
    if sum(abs(contrast_block))~=1
        contrast_block(:)=0;
    end
  end

  if i_contraste >=4 && i_contraste <= 5
    if sum(abs(contrast_block))~=4
        contrast_block(:)=0;
    end
  end
  
  if i_contraste >= 6 && i_contraste <= 11
